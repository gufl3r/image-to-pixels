...
def main(
    ...
    for PixelSequenceFile in glob.glob(f"{Directory}\\*.imgps"):
        ...
        with open(EncodedFrame, "r") as File:
            ...
            Phase = 1
            if SinglePixels != {}:
                PixelIndexes = reversed(SinglePixels.keys())
...
if __name__ == "__main__":
    ...
    for Index, Argument in enumerate(sys.argv):
        ...
        if Argument in ["-t", "--timelapse"]:
            MainArgs["TimelapseArg"] = sys.argv[Index + 1].lower() in [
                "true",
                "yes",
                "y",
                "1",
        ...
        if Argument in ["-ti", "--timelapseinterval"]:
            MainArgs["TimelapseIntervalArg"] = sys.argv[Index + 1]
        if Argument in ["-sp", "--showprogress"]:
            MainArgs["ShowProgressArg"] = sys.argv[Index + 1].lower() in [
                "true",
                "yes",
                "y",
                "1",
        ...
        if Argument in ["-se", "--saveend"]:
            MainArgs["SaveEndArg"] = sys.argv[Index + 1].lower() in [
                "true",
                "yes",
                "y",
                "1",
        ...
        if Argument in ["-s", "--show"]:
            MainArgs["ShowArg"] = sys.argv[Index + 1].lower() in [
                "true",
                "yes",
                "y",
                "1",
